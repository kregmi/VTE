...
def build_model(config):
    ...
    fc_layer = FC_Layer(in_dim=config['in_feat_size'], out_dim=config['d_model'])
    model = FC_Transformer(fc_layer, encoder)

    return model

class FC_Transformer(nn.Module):

    def __init__(self, fc_layer, encoder):
        super(FC_Transformer, self).__init__()

        self.encoder = encoder
        self.fc = fc_layer
        self.maxpool = nn.AdaptiveAvgPool2d((1, self.fc.fc.out_features))

    def forward(self, sources, mask):
        outputs = self.fc(sources)
        outputs = self.encoder(outputs, mask)  # (batch_size, seq_len, d_model)
        return outputs, F.normalize(self.maxpool.forward(outputs), dim=2, p=2)
