...
parser.add_argument('--cachePath', type=str, default='./',
					help='Path to save cache to.')

parser.add_argument('--resume', type=str, default='./runs/',
...
def validate_feats(model, val_loader, val_arg):
	print("Testing: ")
	cityname = val_arg.bdd_gsv_filename_mapper[:-4].split('/')[-1].split('_')[-1]
	model.eval()
	feat_size = 32768
	...
	with torch.no_grad():
		for i, (bdd, gsv, dir_name, lat, lon) in enumerate(val_loader):
			...
			query_feats[i * val_arg.test_num_frames:(i + 1) * val_arg.test_num_frames, :] = feats_bdd.view(-1,
																										   feat_size).cpu().detach().numpy()
			gallery_feats[i * val_arg.test_num_frames:(i + 1) * val_arg.test_num_frames, :] = feats_gsv.view(-1,
																											 feat_size).cpu().detach().numpy()
	...
	sio.savemat('eval_val_' + str(cityname) + '_' + str(avg_error) + '.mat', dict([('sorted_array', idx_sorted_1), ('lat', gallery_lat), ('lon', gallery_lon), ('bdd_feats', query_feats), ('gsv_feats', gallery_feats)]))
...
if __name__ == "__main__":
	...
	data_loader_val_SF = torch.utils.data.DataLoader(val_dataset_SF, batch_size=val_arg_SF.test_batch_size,
													 shuffle=False,
	...
	data_loader_val_NY = torch.utils.data.DataLoader(val_dataset_NY, batch_size=val_arg_NY.test_batch_size,
													 shuffle=False,
													 num_workers=0, drop_last=True)
	...
	data_loader_val_Berkeley = torch.utils.data.DataLoader(val_dataset_Berkeley,
														   batch_size=val_arg_Berkeley.test_batch_size, shuffle=False,
														   num_workers=0, drop_last=True)
	...
	data_loader_val_BayArea = torch.utils.data.DataLoader(val_dataset_BayArea,
														  batch_size=val_arg_BayArea.test_batch_size, shuffle=False,
														  num_workers=0, drop_last=True)
