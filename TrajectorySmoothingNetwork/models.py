...
def build_model(config):
    if config['positional_encoding']:
        source_embedding = PositionalEncoding(
            num_embeddings=config['num_frames'],
    ...
    fc_layer_first = FC_Layer(in_dim=2, out_dim=512)
    fc_layer_last = FC_Layer(in_dim=512, out_dim=2)
    fc_layer_confidence = FC_Layer(in_dim=512, out_dim=2)

    model = FC_Transformer_FC(fc_layer_first, encoder, fc_layer_last, fc_layer_confidence)

    return model

class FC_Transformer_FC(nn.Module):
    def __init__(self, fc_layer_first, encoder, fc_layer_last, fc_layer_confidence):
        super(FC_Transformer_FC, self).__init__()

        self.encoder = encoder
        self.fc1 = fc_layer_first
        self.fc2 = fc_layer_last
        self.fc3 = fc_layer_confidence

    def forward(self, sources, mask=None):
        outputs = self.fc1(sources)
        outputs = self.encoder(outputs, mask)
        deltas = self.fc2(outputs)
        confidences = self.fc3(outputs)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, sources, mask):
        ...
        for encoder_layer in self.encoder_layers:
            ...
            sources = F.normalize(sources.view(-1, self.d_model), dim=1, p=2).view(-1, sources.shape[1], self.d_model)
        return sources
